/**
 * @file sequential_voronoi_output.cc
 ...
 * 
 * This version should NOT be used for timing purposes. This version records and outputs
 * data relating to the operation of the algorithm that greatly slows down the calculation of results.
 * 
...
int		find_Intersection_with_Boundary		(const point p, ordered_vector& direction, timestep_plot_data& plot);
int		find_Endpoint_and_Line				(std::vector<point>::iterator p_it, ordered_vector& direction, std::vector<point>& input_points, timestep_plot_data& plot);
...
int main(int argc, char *argv[]){
	...
	timestep_plot_data plot{10, "plot_info.txt", input_points, boundary_lines};		// struct to record timestep data for timestep plot
	...
	for (auto point_it = input_points.begin(); point_it != input_points.end(); ++point_it){
		...
		plot.clear_green();					// timestep plot
		plot.clear_rays();
		plot.select_point(*point_it);
		
		point p{*point_it};
		if(output) std::cout<<"Calculating cell for point "<<p<<"\n";
		...
		if(output) std::cout<<"Hence our subwedges are:\n\t"<<WedgeQueue[0]<<"\n\t"<<WedgeQueue[1]<<"\n\t"<<WedgeQueue[2]<<WedgeQueue[3]<<"\n";
		if(output) std::cout<<"Beginning WedgeQueue while loop\n";
		/// ------ 0
		...
		while(WedgeQueue.begin() + (long)edge_spot != WedgeQueue.end()){
			num_its++;
			if(output) std::cout<<"\n\nIteration "<<num_its<<" of WedgeQueue while loop ---------------------------------------------------- " << num_its <<"\n";
			if(output) std::cout<<"------------------------------------------------------------------------------------------\n";


			plot.text("consider this wedge");	// adjust header
			plot.set_ray(*point_it, (*point_it) + *(WedgeQueue[edge_spot]).vector1);	// adjust rays to display v
			plot.add_ray(*point_it, (*point_it) + *(WedgeQueue[edge_spot]).vector2);
			plot.increment();


			// find endpoints and bisecting lines
				plot.set_ray(*point_it, (*point_it) + *(WedgeQueue[edge_spot]).vector1);	// adjust rays to display
				plot.text("finding endpoint and line of this ray");	// adjust header
				plot.increment();
				if(output) std::cout<<"Finding endpoint and line of "<<*((WedgeQueue[edge_spot]).vector1)<<" ...\n";
				find_Endpoint_and_Line(point_it, *(WedgeQueue[edge_spot]).vector1, input_points, plot);
				plot.add_green_point(p + (*(WedgeQueue[edge_spot]).vector1).t * *(WedgeQueue[edge_spot]).vector1);	// add the endpoint and line that we found
				plot.add_green_line((*(WedgeQueue[edge_spot]).vector1).boundary);

				plot.set_ray(*point_it, (*point_it) + *(WedgeQueue[edge_spot]).vector2);	// adjust rays to display
				plot.text("finding endpoint and line of this ray");	// adjust header
				plot.increment();
				if(output) std::cout<<"Finding endpoint and line of "<<*(WedgeQueue[edge_spot]).vector2<<" ...\n";
				find_Endpoint_and_Line(point_it, *(WedgeQueue[edge_spot]).vector2, input_points, plot);

				plot.text("now finding intersection...");
				plot.add_green_point(p + (WedgeQueue[edge_spot]).vector2->t * *(WedgeQueue[edge_spot]).vector2);	// add the endpoint and line that we found
				plot.add_green_line((WedgeQueue[edge_spot]).vector2->boundary);
				plot.increment();

				if(output) (WedgeQueue[edge_spot]).vector1->print_all();
				if(output) (WedgeQueue[edge_spot]).vector2->print_all();



			if(output) std::cout<<"Finding determinant of matrix B....   ";
			...
			if(output) std::cout<<determinant;
			...
			if (determinant*determinant < 1e-26){ 
				if (	((WedgeQueue[edge_spot]).vector1->boundary.ycoeff) == 0 		){	// if vertical lines
					...
					if (	(((WedgeQueue[edge_spot]).vector1->boundary.c) / ((WedgeQueue[edge_spot]).vector1->boundary.xcoeff )) == (((WedgeQueue[edge_spot]).vector2->boundary.c) / ((WedgeQueue[edge_spot]).vector2->boundary.xcoeff))	){
						...
						if(output) std::cout<<"\n\tInfinite solutions (same line); move to next edge in WedgeQueue\n";
						plot.text("no intersection becaiuse same line... erasing subwedge");
						plot.increment();
						edge_spot++;
					} else { // no solutions: lines are parallel but not the same
						if(output) std::cout<<"\n\tNo solutions (parallel lines); split edge in half\n";
						// split the edge in half
						// new ray is
						// double r1 = 0;
						double r2 = 1;
						plot.text("no intersection becaiuse parallel lines... adding subwedge");
					...
					if(output) std::cout<<"\n\tNo solutions (parallel lines); split edge in half\n";
					plot.text("no intersection becaiuse parallel lines... splitting wedge");
					// split the edge in half
					// new ray is
					double r1, r2;
			...
			// CASE 2: unique solution
				...
				plot.text("Unique solution here");
				plot.add_x(intersection);
				plot.increment();
				if(output) std::cout<< intersection<<"\n";

				if ( (lambda1 < 0) || (lambda2 < 0)){	// if unique solution is not in the subwedge
					if(output) std::cout<<"This is not in the wedge! Splitting subwedge...\n\t";
					...
					plot.text("Not in wedge... splitting wedge");
					plot.add_ray(p, p+rays.back());
					plot.increment();


				} else {	// the unique solution IS in the subwedge
					if(output) std::cout<<"This is in the wedge ! Determining whether it is in cell...";
					...
					for (auto it = input_points.begin(); it != input_points.end(); ++it){		// loop through input points and find closest input point to endpoint and corresponding distance
						...
						if ( check_dist < min_distance){	// if we have found a closer point
							if ((check_dist - min_distance)*(check_dist - min_distance) > 1e-20){	// make sure that we aren't considering any points on the circle centred at intersection and passing through p
								if(output) std::cout<<"NO\n\tThe input point "<< (*it) <<" is closer by "<< (check_dist - min_distance)  << "\n\t";
								plot.add_x(*it);
					...
					if (in_cell) {	// we have found a vertex
						...
						if(output) std::cout<<"Checking that point is in region...\n\t ";
						if (in_region(intersection)){	// if we are in the world...
							if(output) std::cout<<"YES\nFOUND VERTEX !\n\tPoint: "<<intersection<<"\n\tLine 1: "<<(WedgeQueue[edge_spot]).vector1->boundary<<"\n\tLine 2: "<<(WedgeQueue[edge_spot]).vector2->boundary<<"\n";
							
							// save vertex data
							cells.back().add_vertex(intersection, (WedgeQueue[edge_spot]).vector1->index, (WedgeQueue[edge_spot]).vector1->boundary);

							plot.text("intersection in wedge AND in cell ! new vertex found");
							plot.add_vertex(intersection);
							plot.increment();
							...
							if(output) std::cout<<"NO\n";
							if(output) std::cout<<"Splitting subwedge...\n\t";
							plot.text("intersection is not in region. splitting subwedge...");
						...
						if(output) std::cout<<"Splitting subwedge...\n\t";
						plot.text("intersection in wedge but NOT in cell. splitting subwedge...");
		...
		if (output) std::cout<<"\n\nCompleted cell for point " << p <<"\n\nPrinting sorted cell data: ---------------------------------------------------------------------------------\n\n";
		cells.back().sort_vertices();
		if (output) std::cout<<cells.back();

		// add cell wall information to plot
		for (int i = 0; (size_t) i < cells.back().vertices.size()-1; i++){
			auto it = cells.back().vertices.cbegin() + i;
			plot.add_wall(*it, *(it+1));
		}
		plot.add_wall(cells.back().vertices.front(), cells.back().vertices.back());
	...
	std::cout<<"\nProgram complete\nSee results_seq_" + std::to_string(numberOfPoints) + ".txt for results\nand see plot_info.txt for timestep plot information.\n";
	std::cout<<"\nThe file results_seq_" + std::to_string(numberOfPoints) + ".txt can be read by the python matplotlib script Results_Plot.py\n";
	std::cout<<"and the file plot_info.txt can be read by the python matplotlib script Voronoi_Plot_Slider.py\n";
	std::cout<<"(It is recommended to use a small number of input points (~10) for viewing algorithm operation in the timestep plot.)\n";
...
int readBoundaryLines(){
	if (output) std::cout<<"Reading boundary lines from file "<<boundary_filename<<"\n";
	...
	if (output) std::cout<<"Number of boundary lines: "<<numberOfLines<<"\n";
	// read lines
	if (output) std::cout<<"Reading boundary lines...\n";
 ...
 */
int generateDefaultBoundary(std::vector<point>& points){
	if (output) std::cout<<"Generating default boundary lines with spacing size = "<<spacing<<"\n";
	...
	if (output) std::cout<<"Extreme x and y coordinates are:\n\tx minimum: "<<minx<<"\n\tx maximum: "<<maxx<<"\n\ty minimum: "<<miny<<"\n\ty maximum: "<<maxy<<"\n";
...
void printUsage(void){
	...
	std::cout<<"        -o              : will cause the program to display output revealing the operation of the algorithm\n";
...
int find_Endpoint_and_Line(std::vector<point>::iterator p_it, ordered_vector& direction, std::vector<point>& input_points, timestep_plot_data& plot){
	...
	if (output) std::cout<<"find_Endpoint_and_Line()\n\tChecking if we have already computed endpoint...";
	if (direction.t > 0){
		if (output) std::cout<<"  YES done\n";
		plot.text("we have already found the endpoint and line here");
		plot.add_x(p + direction.t*direction);
		plot.add_line(direction.boundary);
		plot.set_ray(p, p + direction.t*direction);
		plot.increment();
		return 0;
	}
	if (output) std::cout<<"  NO\n\t";
	...
	if (output) std::cout<<"Must find intersection with boundary... calling\n\tfind_Intersection_with_Boundary()\n\t\t";
	find_Intersection_with_Boundary(p, direction, plot);
	plot.add_x(p+direction.t*direction);	// current proposed endpoint
	if (output) std::cout<<"\n\tIntersection with boundary is at "<<p<<" + " <<direction.t << " * " << direction<<"   at boundary line "<< direction.boundary<<"\n\t";
	if (output) std::cout<<"Now searching for cell endpoint...\n\t";
	bool foundEndpoint = false;
	std::vector<std::vector<point>::iterator> skip_items (1, p_it);
	while (! foundEndpoint){
		point endpoint = p + (direction.t * direction);	// this is our endpoint
		if (output) std::cout<<"checking whether proposed endpoint "<<endpoint<<" is in the cell...\n\t";
		...
		if (output) std::cout<<"min distance is "<<min_distance<<"\n\t";
		...
		for (auto it = input_points.begin(); it != input_points.end(); ++it){		// loop through input points and find closest input point to endpoint and corresponding distance
			...
			if ( current_distance < min_distance){	// if we have found a closer point
				...
				if (!skip){	// make sure that we aren't considering p itself as a closer input point
					...
					if (output) std::cout<<"\tPoint "<<(*it)<< " is closer (" << current_distance <<")\n\t";
					closeNeighbour = it;
					in_cell = false;	// we have found a closer input point, hence we are not in the cell yet
				}
			}
		...
		if (in_cell){	// we have found the endpoint and we are done
			plot.text("checking whether endpoint in cell...YES");
			plot.increment();
			foundEndpoint = true;
			if (output) std::cout<<"Found endpoint: "<< p + (direction.t * direction) <<"\n";
			if (output) std::cout<<"\tLine is: "<< direction.boundary <<"\n";
		} else {		// we have not found the endpoint; there is a closer input point to the current suspected endpoint
			plot.add_x(*closeNeighbour);
			plot.text("checking whether endpoint in cell...NOPE closeNeighbour");
			plot.keep_all();
			...
			// suspected boundary lines becomes the perpedicular bisector between p and closeNeighbour
				...
				plot.add_line(direction.boundary);
				plot.text("new suspected wall line is bisector and point is intersection with ray");
			...
			// suspected endpoint is now the intersection between the perpendicular bisector and ray from p
				...
				plot.add_x(p +direction.t *direction);
				plot.keep_all();
			if (output) std::cout<<"Suspected endpoint is now " << p + (direction.t * direction) <<" on bisector "<< direction.boundary <<" with closeNeighbour "<< (*closeNeighbour) << "\n\t";
...
int find_Intersection_with_Boundary(const point p, ordered_vector& direction, timestep_plot_data& plot){
	double ti;
	plot.text("searching for intersection with boundary...");

	// cycle through halfplanes in boundary
	if (output) std::cout<<"Searching for first intersection ...\n\t\t";
	...
	while ((! foundFirstIntersection) && (it != boundary_lines.end())){	// first we find an initial intersection of the ray with boundary
		if (output) std::cout<<"testing halfplane "<<(*it)<<"...";
	...
	if (output) std::cout<<"Now we search for a closer intersection amongst remaining halfplanes...\n\t\t";
	while (it != boundary_lines.end()){	// now we iterate through the remaining boundary lines and see if we can find a closer intersection
		...
		if ( parallel_check != 0 ){	// if boundary line and direction vector are not parallel
			...
			if ((ti > 0) && (ti < direction.t)){	// if ray intersects boundary line AND at a point closer than before
				if (output) std::cout<<"Closer intersection! t = "<<ti;
	...
	plot.set_ray(p, p+direction.t*direction);
	plot.text("found intersection with boundary!");
	plot.increment();
	// done !
	return 0;
}
...
int in_region(point pt){
	...
	for (auto it = boundary_lines.cbegin(); it != boundary_lines.cend(); ++it){
		if ((*it) * pt > (*it).c + 1e-10){
			if (output) std::cout<<"line: "<<(*it)<<"\tand ax+by = "<< std::setprecision(20)<<(*it) * pt<<std::setprecision(5)<<"\n";
			return 0;
		}
	}
	return 1;
