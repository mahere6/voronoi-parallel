/**
 * @file complex_parallel_voronoi_output.cc
 ...
 * 
 * This version should not be used for timing purposes, as it takes time to print
 * extra plot information files for revealing individual process operation throughout
 * the algorithm.
 * 
...
void	whats_going_on_pt1		(int myid, std::string point_filename, std::vector<point>& input_points, std::vector<cell_info>& cells);
void	whats_going_on_pt2		(	int myid, std::vector<double *>& recieved_giveups, std::vector<double *>& recieved_backups, std::vector<int>& number_of_giveups_to_recieve, std::vector<size_t>& source_indices, std::vector<int>& number_of_backups_to_recieve, std::vector<cell_info>& cells);
void	whats_going_on_pt3		(int myid, std::vector<cell_info>& cells);
...
void	compute_remaining_cells						(int myid, std::vector<point>& input_points, std::vector<line>& boundary_lines, std::vector<double *>& recieved_giveups, std::vector<int>& number_of_giveups_to_recieve, std::vector<size_t>& source_indices, std::vector<double *>& recieved_backups, std::vector<int>& number_of_backups_to_recieve, std::vector<cell_info>& mycells, std::vector<unsigned int>& giveup_indices, double * giveups_time, double * recompute_time);
...
int main(int argc, char *argv[]){
	...
	if (rank == 0) std::cout << "Range of all points: " << min[0] << " <= x <= " << max[0] << ", " << min[1] << " <= y <= " << max[1] << std::endl;
	...
	whats_going_on_pt1(myid, points_filename, mypoints, mycells);
	...
	switch(num_sides_to_send_to){
		...
		case 1:
		{
			...
			compute_remaining_cells(myid, mypoints, boundary_lines, recieved_giveups, number_of_giveups_to_recieve, source_indices, recieved_backups, number_of_backups_to_recieve, mycells, giveup_indices, &giveups_time, &recompute_time);

			whats_going_on_pt3(myid, mycells);
	...
	if (myid != 0){
		...
		std::cout<<"The output files of the form what_is_going_on_rank*.txt reveal individual process operation and can be read by the python matplotlib script What_Is_Going_On_Plot.py\n";
...
int parseArguments(int argc, char * argv[], int rank, int size, char ** points_filename, double * spacing, int * xdim, int * ydim){
	...
	while ((opt = getopt(argc, argv, "p:id:hx:o:")) != -1){
		switch(opt){
			case 'p':
 ...
 *	
 *	@param [in] giveup_buffer An array of doubles representing points which are "giveups" to the other process.
...
void send4(std::vector<double *>& backup_buffers, std::vector<unsigned int>& backup_index, bool * send_info, std::vector<point>& input_points, std::vector<size_t>& counters, int myid){
	...
	if (myid == print_rank) std::cout<<"Rank "<<myid<<": done sorting data for sending\n";
...
void compute_remaining_cells(int myid, std::vector<point>& input_points, std::vector<line>& boundary_lines, std::vector<double *>& recieved_giveups, std::vector<int>& number_of_giveups_to_recieve, std::vector<size_t>& source_indices, std::vector<double *>& recieved_backups, std::vector<int>& number_of_backups_to_recieve, std::vector<cell_info>& mycells, std::vector<unsigned int>& giveup_indices, double * giveups_time, double * recompute_time){
	...
	whats_going_on_pt2(myid, recieved_giveups, recieved_backups, number_of_giveups_to_recieve, source_indices, number_of_backups_to_recieve, mycells);
...
/**
 *	\brief This function is the first of a series of three functions that prints
 *	information (to file) about the operation of a particular rank throughout the
 *	complex parallel Voronoi algorithm. The file produced is named e.g. "whats_going_on_rank4.txt"
 *	for a process with rank 4 and can be read by the python matplotlib script What_Is_Going_On_Plot.py,
 *	which will produce a timestep plot for the operation of e.g. rank 4.
 *
 *	Timestep 0 displays the initial points assigned to the process;
 *	Timestep 1 displays the locally-generated cells for the points that were not giveups;
 *	Timestep 2 displays the giveups received from neighbour processes;
 *	Timestep 3 displays the backups received from neighbour processes;
 *	Timestep 4 displays the cells computed for received giveups (as well as the previously computed local cells);
 *	Timestep 5 displays the cells computed for received giveups and recomputed initially-held cells.
 *
 *	This function writes intially assigned points and initially computed cells to file.

 *	@param [in] myid The rank of the calling process
 *	@param [in] point_filename The filename (supplied by the user) from which input points were read
 *	@param [in] input_points The std::vector<point> of points initially assigned to the calling process
 *	@param [in] cells The std::vector<cell_info> of cells locally determined
 */
void whats_going_on_pt1(int myid, std::string point_filename, std::vector<point>& input_points, std::vector<cell_info>& cells){

	// first write input points to file
	std::ofstream plot_file;

	std::string mynum = std::to_string(myid);
	std::string local_filename = "whats_going_on_rank" + mynum + ".txt";

	plot_file.open(local_filename);
	if (!plot_file){	// error checking
		std::cerr<<"Failed to open file for writing plot data for Voronoi diagram";
		return;
	}

	plot_file << "FNAME "<< point_filename << "\n";		// print filename to read all background points from

	for (auto it = input_points.begin(); it != input_points.end(); ++it){
		plot_file<<"MYPOINT " << " " << (*it).x << " " << (*it).y << "\n";		// format: BKGDP 0.487 8.001
	}
	

	// now write initial cell data
	int i = 0;
...
/**
 *	\brief This function is the second of a series of three functions that prints
 *	information (to file) about the operation of a particular rank throughout the
 *	complex parallel Voronoi algorithm. The file produced is named e.g. "whats_going_on_rank4.txt"
 *	for a process with rank 4 and can be read by the python matplotlib script What_Is_Going_On_Plot.py,
 *	which will produce a timestep plot for the operation of e.g. rank 4.
 ...
 *	Timestep 5 displays the cells computed for received giveups and recomputed initially-held cells.
 *
 *	This function writes the received giveup and backup points to file, and write the cells computed for received giveups (and initial unadjusted cells) to file.

 *	@param [in] myid The rank of the calling process
 *	@param [in] recieved_giveups A std::vector<double *> of double arrays representing giveup points received from separate processes
 *	@param [in] recieved_backups A std::vector<double *> of double arrays representing backup points received from separate processes
 *	@param [in] number_of_giveups_to_recieve A std::vector<int> holding the number of giveups we received from each process
 *	@param [in] source_indices A std::vector<size_t> holding numbers in the range [0,7] that represent which neighbours we received from
 *	@param [in] number_of_backups_to_recieve A std::vector<int> holding the number of backups we received from each process
 *	@param [in] cells The std::vector<cell_info> of cells locally determined
 */
void whats_going_on_pt2(int myid, std::vector<double *>& recieved_giveups, std::vector<double *>& recieved_backups, std::vector<int>& number_of_giveups_to_recieve, std::vector<size_t>& source_indices, std::vector<int>& number_of_backups_to_recieve, std::vector<cell_info>& cells){
	...
	std::string mynum = std::to_string(myid);
	std::string local_filename = "whats_going_on_rank" + mynum + ".txt";
	plot_file.open(local_filename, std::ios::app);
	if (!plot_file){	// error checking
		std::cerr<<"Failed to open file for writing plot data for Voronoi diagram";
		return;
	}
	...
	size_t current_source = 0;
	// print recieved giveups
	for (auto buffer_it = recieved_giveups.begin(); buffer_it != recieved_giveups.end(); ++buffer_it){
		for (int point_spot = 0; point_spot < number_of_giveups_to_recieve[source_indices[current_source]]*2; point_spot+=2){
			plot_file << "RG " << (*buffer_it)[point_spot] << " " << (*buffer_it)[point_spot+1] << "\n";
		}
		current_source++;
	}

	current_source = 0;
	// print recieved backups
	for (auto buffer_it = recieved_backups.begin(); buffer_it != recieved_backups.end(); ++buffer_it){
		for (int point_spot = 0; point_spot < number_of_backups_to_recieve[source_indices[current_source]]*2; point_spot+=2){
			plot_file << "RB " << (*buffer_it)[point_spot] << " " << (*buffer_it)[point_spot+1] << "\n";
		}
		current_source++;
	}


	// print intermediate cells
	int i = 0;
	for (auto cell_it = cells.begin(); cell_it != cells.end(); ++cell_it){
		// sort the cell
		(*cell_it).sort_vertices();
...
/**
 *	\brief This function is the third of a series of three functions that prints
 *	information (to file) about the operation of a particular rank throughout the
 *	complex parallel Voronoi algorithm. The file produced is named e.g. "whats_going_on_rank4.txt"
 *	for a process with rank 4 and can be read by the python matplotlib script What_Is_Going_On_Plot.py,
 *	which will produce a timestep plot for the operation of e.g. rank 4.
 ...
 *	Timestep 5 displays the cells computed for received giveups and recomputed initially-held cells.
 *
 *	This function writes the final cells computed by the calling process.

 *	@param [in] myid The rank of the calling process
 *	@param [in] cells The std::vector<cell_info> of cells finally determined
 */
void whats_going_on_pt3(int myid, std::vector<cell_info>& cells){
